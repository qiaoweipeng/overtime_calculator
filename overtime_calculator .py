...
def calculate_overtime(input_excel_path, sheet_name='深圳', output_dir='./output'):
    """
    计算Excel考勤表中指定工作表的加班时长，并生成核对报告。

    Args:
        input_excel_path (str): 输入Excel文件的路径。
        sheet_name (str): 需要处理的工作表名称，默认为'深圳'。
    ...
    # 复制原文件，不修改原始文件
    output_excel_path = os.path.join(output_dir, os.path.basename(input_excel_path).replace(".xlsx", "_加班核对版.xlsx"))
    ...
    for header in headers:
        if header and isinstance(header, str):
            # 匹配类似 "05-01 星期五" 或 "05-01星期五" 的格式
            match = re.search(r'^(\d{2})-\d{2}', header.strip())
            if match:
    ...
    # 3. 如果没识别到，抛出错误
    if not target_month:
        print("❌ 错误：无法在表头中自动识别月份，请检查列名格式是否包含 'XX-XX'。")
        return
    # --- 自动识别月份 END ---
    ...
    if not date_cols:
        print("错误：未找到日期列（如'03-01 星期日'）。请检查Excel文件格式。")
        return
    if not overtime_total_col:
        print("警告：未找到'加班总时长/小时'列。将无法进行差异核对。")
    ...
    title_headers = ['序号', '姓名', '部门', '计算加班(小时)', '表格已有加班(小时)', '差异(小时)', '核对结果']
    col_widths = [6, 10, 20, 16, 18, 12, 12]
    ...
    for i, r in enumerate(results):
        row_num = i + 2
        fill = alt_row_fill if i % 2 == 0 else PatternFill(fill_type=None)
        ...
        for col, val in enumerate(values, 1):
            ...
            if col in [4, 5, 6]: # 小时数和差异格式化
                cell.number_format = '0.00'

    # ---- Sheet 2: 逐日加班明细 ----
    ws_detail = wb.create_sheet('逐日加班明细')
    
    detail_headers = ['姓名', '部门', '日期', '下班时间', '原始加班(分钟)', '计算加班(分钟)', '计算加班(小时)', '原始数据']
    detail_widths = [10, 20, 15, 10, 15, 15, 14, 50]
        ...
        for d in r['daily_details']:
            if d['minutes'] > 0:
                ...
                for col, val in enumerate(values, 1):
                    ...
                    cell.border = thin_border
                    cell.alignment = center_align if col not in [2, 8] else left_align
                    cell.fill = fill
                ws_detail.cell(row=detail_row_idx, column=7).number_format = '0.00'
                detail_row_idx += 1

    # ---- Sheet 3: 差异项目 ----
    ws_diff = wb.create_sheet('差异项目')
    
    diff_headers = ['序号', '姓名', '部门', '计算加班(小时)', '表格已有(小时)', '差异(小时)', '差异说明']
    diff_widths = [6, 10, 20, 16, 16, 12, 50]
    ...
    if not diff_items:
        cell = ws_diff.cell(row=2, column=1, value='所有人员加班时长核对一致，无差异项目')
        cell.font = Font(color='008000', bold=True)
        ws_diff.merge_cells('A2:G2')
        cell.alignment = center_align
    else:
        for i, r in enumerate(diff_items):
            ...
            for col, val in enumerate(values, 1):
                cell = ws_diff.cell(row=row_num, column=col, value=val)
                cell.border = thin_border
                cell.alignment = center_align if col not in [3, 7] else left_align
                cell.fill = diff_fill_light_red
                if col in [4, 5, 6]:
                    cell.number_format = '0.00'
    ...
    # 标题
    title_cell = ws_help.cell(row=1, column=1, value='深圳考勤加班核对 — 使用说明')
    ...
    # 生成Markdown报告
    markdown_report_path = os.path.join(output_dir, '深圳加班核对报告.md')
    with open(markdown_report_path, 'w', encoding='utf-8') as f:
        f.write("# 深圳考勤加班时长核对报告\n\n")
        f.write("**核对日期：** {}\n".format(datetime.now().strftime("%Y年%m月%d日")))
        f.write("**数据来源：** {}\n".format(os.path.basename(input_excel_path)))
        f.write(f"**核对范围：** 2025年{check_date_range}\n")
        f.write("**核对人数：** {}\n\n---\n\n".format(len(results)))
        f.write("## 核对规则说明\n\n")
        f.write("| 规则项目 | 说明 |\n")
        f.write("|---------|------|\n")
        ...
        for i, r in enumerate(results):
            ...
            f.write(f"| {i+1} | {r['name']} | {r['dept']} | {r['total_hours']:.2f} | {existing_val} | {diff_str} | {'❌ 有差异' if is_diff else '✓ 一致'} |\n")
        f.write("\n---\n\n")

        if diff_items:
            f.write("## 差异项目详情\n\n")
            f.write("| 序号 | 姓名 | 部门 | 计算加班(小时) | 表格已有(小时) | 差异(小时) | 差异说明 |\n")
            f.write("|-----|------|------|------------|------------|------------|----------|\n")
            for i, r in enumerate(diff_items):
                f.write(f"| {i+1} | {r['name']} | {r['dept']} | {r['total_hours']:.2f} | {r['existing_float']:.2f} | {r['diff']:.2f} | 计算值{r['total_hours']:.2f}h，表格值{r['existing_float']:.2f}h，差异{r['diff']:.2f}h |\n")
            f.write("\n---\n\n")

        # 加班时长较多人员（Top 10）
        sorted_results = sorted(results, key=lambda x: x['total_hours'], reverse=True)
        f.write("## 加班时长较多人员（Top 10）\n\n")
        f.write("| 排名 | 姓名 | 部门 | 加班总时长(小时) |\n")
        f.write("|-----|------|------|-------------|\n")
        for i, r in enumerate(sorted_results[:10]):
            f.write(f"| {i+1} | {r['name']} | {r['dept']} | {r['total_hours']:.2f} |\n")
        f.write("\n---\n\n")

        f.write("*报告由自动化脚本生成，核对逻辑：提取每日最后一次打卡时间，计算19:00后加班时长，以30分钟为单位向下取整（不足30分钟不计）。*\n")
...
if __name__ == '__main__':
    ...
    calculate_overtime(input_file, sheet_name=sheet_to_process, output_dir=output_folder)
